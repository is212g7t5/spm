        ...
        <li className="flex flex-row">
          <div className="select-none cursor-pointer hover:bg-gray-50 flex flex-1 items-center p-4">
            ...
            <div className="flex flex-row justify-center">
              {/* Create LJ button */}
              <button type="button"
                className="text-white bg-orange-500 hover:bg-orange-600 focus:ring-4 font-medium rounded-lg text-sm px-5 py-2.5 text-center m-1" onClick={() => createLJ(jobId)}>
                <span>Create Learning Journey</span>
              </button>
            </div>
            <div className="flex flex-row justify-center">
              <button className="w-10 text-right flex justify-end" type="button" onClick={() => setIsDetailsOpen(!isDetailsOpen)}>
                {isDetailsOpen ? <ChevronDownIcon className="w-5 h-5" aria-hidden="true" /> : <ChevronRightIcon className="w-5 h-5" aria-hidden="true" />}
              </button>
            </div>
          </div>
        </li>
