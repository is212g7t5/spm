...
from app.tests.utils.job_skill import random_numbers


def test_create_job_skill(db: Session) -> None:
    job_id = random_numbers(20)
    skill_id = random_numbers(20)
...
def test_get_job_skill(db: Session) -> None:
    ...
    stored_job = crud.job_skill.get(db=db, job_id=job_skill.job_id)
    assert stored_job
...
def test_update_job_skill(db: Session) -> None:
    job_skill = create_random_job_skill(db)
    new_job_id = random_numbers(20)
    new_skill_id = random_numbers(20)
    assert new_job_id != job_skill.new_job_id
    assert new_skill_id != job_skill.new_skill_id
