...
def get_all_learning_journey_course(
    ...
    """
    learning_journey = crud.learning_journey_course.get_by_lj_id(db, lj_id=lj_id, skip=skip)
...
def create_learning_journey_course(
    *,
    ...
    """
    learning_journey_course = crud.learning_journey_course.get(db, lj_id=lj_id, course_id=course_id)
    if learning_journey_course:
        raise HTTPException(
            status_code=401, detail="Learning journey has been assigned this course in database"
        )
    learning_journey_course_in = schemas.LJCourseCreate(
        lj_id=lj_id,
        course_id=course_id,
    )
    learning_journey_course = crud.learning_journey_course.create(db, obj_in=learning_journey_course_in)
    return learning_journey_course


@router.delete("/{lj_id}&{course_id}", response_model=schemas.SkillCourse)
def delete_learning_journey_course_by_id(
    *,
    ...
    learning_journey_course = crud.learning_journey_course.remove(db, lj_id=lj_id, course_id=course_id)
    return learning_journey_course
